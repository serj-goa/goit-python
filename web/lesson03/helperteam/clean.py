...
def clean_main(folder_path: 'Path') -> None:
    MAIN_USER_PATH = folder_path
    sorted_files_by_category = dict()
    all_files_in_user_path = iterate_over_folder(MAIN_USER_PATH)
    ...
    for file_path in all_files_in_user_path:
        ...
        if not folder_name_for_moving_file in sorted_files_by_category:
            sorted_files_by_category[folder_name_for_moving_file] = [
                new_file_name]
        else:
            sorted_files_by_category[folder_name_for_moving_file].append(
                new_file_name)

        if not file_extension in ALL_TYPES['Archives']:
            valid_uniq_name = get_valid_uniq_name(
                new_category_path, new_file_name)
            ...
            folder_name_for_unpack = get_valid_uniq_name(
                new_category_path, normalize_name)
    ...
    for folder in MAIN_USER_PATH.iterdir():
        if len(os.listdir(folder)) == 0:
            shutil.rmtree(folder, ignore_errors=True)

    print_result(sorted_files_by_category,
                 all_famous_extensions, unknown_extensions)
